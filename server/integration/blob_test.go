...
// actually uses it (a plain POST straight to S3, never proxied through the
// relay — bytes stay off the relay's own compute and bandwidth, and the
// presigned URL is still gated behind the same write-token check that
// mints it), then read back through the relay's own redirect and deleted
// through it. internal/storage/blobstore/s3's own
