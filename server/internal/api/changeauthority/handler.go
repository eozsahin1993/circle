...
type request struct {
	...
	// EncryptedMeta is base64-encoded ciphertext — the role_change entry's payload. Opaque to this handler.
	EncryptedMeta string `json:"encryptedMeta"`
	// KeyVersion: nothing rotates here, so unlike rotate there's no pre/post distinction.
	KeyVersion int64 `json:"keyVersion"`
	// WriteToken: raw hex, proves current membership.
	WriteToken string `json:"writeToken"`
	Action     string `json:"action"`
	// TargetAuthorityPublicKey: hex ed25519 key entering or leaving the set.
	...
	// Signature: hex, over logstore.AuthorityChangeMessage(action, syncId, entryId, targetAuthorityPublicKey).
...
func (h *Handler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	for field, value := range map[string]string{
		...
		"action":                   req.Action,
		"targetAuthorityPublicKey": req.TargetAuthorityPublicKey,
	...
	result, err := h.Service.ChangeAuthority(r.Context(), logstore.AuthorityChange{
		SyncID:                   syncID,
		EntryID:                  req.EntryID,
		EncryptedPayload:         encryptedMeta,
		KeyVersion:               req.KeyVersion,
		WriteToken:               req.WriteToken,
		Action:                   logstore.AuthorityAction(req.Action),
		TargetAuthorityPublicKey: req.TargetAuthorityPublicKey,
		SignerAuthorityPublicKey: req.SignerAuthorityPublicKey,
		Signature:                signature,
	})
