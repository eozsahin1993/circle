// Package circleerrors maps synclog's sentinel errors to HTTP status
...
func Status(err error) (int, string) {
	switch {
	case errors.Is(err, synclog.ErrCircleNotFound):
		return http.StatusNotFound, "circle not found"
	case errors.Is(err, synclog.ErrAlreadyExists):
		return http.StatusConflict, "circle already exists"
	case errors.Is(err, synclog.ErrInvalidNamespace):
		return http.StatusBadRequest, "invalid namespace"
	case errors.Is(err, synclog.ErrWriteTokenMismatch):
	...
	case errors.Is(err, synclog.ErrBlobAlreadyExists):
		...
		// synclog.BlobStore.GetUploadTarget's doc comment.
