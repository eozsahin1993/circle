...
type request struct {
	...
	// Signature: hex, over synclog.AuthorityChangeMessage(action, syncId, entryId, targetAuthorityPublicKey).
...
func (h *Handler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	result, err := h.Service.ChangeAuthority(r.Context(), synclog.AuthorityChange{
		SyncID:                   syncID,
		EntryID:                  req.EntryID,
		EncryptedPayload:         encryptedMeta,
		KeyVersion:               req.KeyVersion,
		WriteToken:               req.WriteToken,
		Action:                   synclog.AuthorityAction(req.Action),
		TargetAuthorityPublicKey: req.TargetAuthorityPublicKey,
		SignerAuthorityPublicKey: req.SignerAuthorityPublicKey,
		Signature:                signature,
	})
