...
func (c circle) uploaderSignature() string {
	return hex.EncodeToString(ed25519.Sign(c.uploaderPriv, synclog.DeleteBlobMessage(c.syncID, entryID)))
}

func (c circle) gone(t *testing.T) bool {
	t.Helper()
	_, err := c.blobStore.UploaderPublicKey(context.Background(), c.syncID, entryID)
	return errors.Is(err, synclog.ErrBlobNotFound)
}
...
func TestService_Delete_RefusesASignatureFromAnyOtherKey(t *testing.T) {
	...
	forged := hex.EncodeToString(ed25519.Sign(strangerPriv, synclog.DeleteBlobMessage(c.syncID, entryID)))
...
func TestService_Delete_AllowsAnAdminWhoDidNotUpload(t *testing.T) {
	c := newCircle(t)
	signature := ed25519.Sign(c.founderPriv, synclog.DeleteBlobMessage(c.syncID, entryID))
...
func TestService_Delete_RejectsAnAuthorityKeyNotInTheAuthoritySet(t *testing.T) {
	...
	signature := ed25519.Sign(strangerPriv, synclog.DeleteBlobMessage(c.syncID, entryID))
...
func TestService_Delete_RejectsASignatureOverAnotherEntry(t *testing.T) {
	c := newCircle(t)
	signature := ed25519.Sign(c.founderPriv, synclog.DeleteBlobMessage(c.syncID, "some-other-entry"))
