...
func (s *Service) Delete(
	...
	if errors.Is(err, synclog.ErrBlobNotFound) {
		return nil
	}
	if err != nil {
		return err
	...
	if !verifiesAsUploader(uploaderPublicKey, uploaderSignature, syncID, entryID) {
		...
		if err := s.LogStore.VerifyAuthoritySignature(
			...
			syncID,
			authorityPublicKey,
			synclog.DeleteBlobMessage(syncID, entryID),
			authoritySignature,
		); err != nil {
			return err
		}
...
func verifiesAsUploader(publicKeyHex, signatureHex, syncID, entryID string) bool {
	...
	return ed25519.Verify(publicKey, synclog.DeleteBlobMessage(syncID, entryID), signature)
}
