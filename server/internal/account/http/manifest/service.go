...
//
// See account's package doc: this endpoint's one-blob-per-account
...
func (s *Service) Get(ctx context.Context, accountID string) (account.Manifest, error) {
	return s.ManifestStore.GetManifest(ctx, accountID)
}

// Put replaces accountID's manifest, only if it's still at expectedVersion —
// returns account.ErrVersionMismatch if another device wrote first.
