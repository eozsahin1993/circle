...
import { circleNotificationChannelId, ensureCircleNotificationChannel, removeCircleNotificationChannel } from '@/features/push-notifications/services/channels';
...
test('every circle is a channel under the one group', async () => {
  await ensureCircleNotificationChannel('circle-1', 'Family Circle');
  await ensureCircleNotificationChannel('circle-2', 'Book Club');
...
test('renaming updates the same channel', async () => {
  await ensureCircleNotificationChannel('circle-1', 'Family Circle');
  await ensureCircleNotificationChannel('circle-1', 'Nana House');
